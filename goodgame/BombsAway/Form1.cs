...
namespace BombsAway
{
    public partial class Form1 : Form
    {
            ...
            switch (e.KeyCode)
            {
                ...
                case Keys.P:                    // On P Keypress down
                    if (GameOn)
                    {
                        ...
                        label_Dead.Text = "Пауза, нажмите З чтобы продолжить";
                        label_Dead.Visible = true;
                    }
                    else
                    {
                        GameOn = true;          //Game resumes, death text changes back
                        label_Dead.Text = "Вы мертвы, нажмите пробел чтобы начать заного";
                        label_Dead.Visible = false;
                    }
                    break;
                ...
                case Keys.Space:    // On Space Keypress down
                    if (label_Dead.Visible && !label_Dead.Text.Contains("Пауза"))
        ...
        private void timer_Anim_Tick(object sender, EventArgs e)
        {
            ...
            label1.Text = "Бомбы: " + GetBombsNum(Bombs); //Sets the number of bombs label
            label2.Text = "Рекорд: " + Properties.Settings.Default.Highscore;
